...
"""
There are two main difference between the current detection head and the original YOLOX head:
(1) We use 4 convs (instead of 2 conv) in detection heads
(2) We adding an attention block to enlarge the effective receptive field
These changes bring better detection results and faster convergence.
Add CondInst for instance segmentation
"""
class YOLOXHeadDetMask(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
    ...
    def forward(self, xin, labels=None, imgs=None, masks=None):
        """masks: (bs, M, H//4, W//4)"""
        outputs = []
        origin_preds = []
        x_shifts = []
